...
def post_detail(request, year, month, day, post):
    post = get_object_or_404(Post.published,
                             ...
                             publish__day=day)
    # list of active comment for a particular post
    comments = post.comments.filter(active=True)
    # create form for user to comment
    form = CommentForm()
...
@require_POST #this allows only post form methon for this view...
def post_comment(request, post_id):
    ...
    if form.is_valid():
        ...
        # assign the comment to a post
        comment.post = post
        # save the comment to database now
        comment.save()
